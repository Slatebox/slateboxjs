...
class $f9b2caafbe71c9e4$export$2e2bcd8739ae039 {
    constructor(slate, node){
        ...
        self.dragEvents = {
            move (dx, dy) {
                if (!self.slate.isCtrl) self.enactMove(dx, dy);
            },
            async up () {
                if (!self.slate.isCtrl) self.finishDrag(true);
            },
            dragger (x, y, e) {
                if (!self.slate.canvas.isDragging) {
                    self.slate.nodes.closeAllMenus();
                    console.log('state of indiv select', self.slate.isCtrl, self.slate.candidatesForSelection, self.node.options.id);
...
class $2c56d294fa5e840e$export$2e2bcd8739ae039 {
    ...
    refreshMarker(blnIndiv) {
        ...
        if (blnIndiv) self.selectedNodes.forEach((n)=>{
            n.mark();
        });
    }
